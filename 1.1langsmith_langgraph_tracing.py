...
# --- Models ---
# For LangChain segments (also traced if env configured)
...
def leadgen_pipeline(role_query: str, max_leads: int = 3) -> List[Dict[str, Any]]:
    """End-to-end: search → parse → RAG company summary → score → email."""
    ...
    for hit in leads:
        parsed = infer_name_title_company(hit["title"] or hit["snippet"])
        lead = {
            "name": parsed.get("name") or hit["title"].split(" - ")[0],
        ...
        # 2) Get company pages (about / product), fetch text, build small context
        docs = []
        if lead["company"]:
            for u in search_company_pages(lead["company"], max_urls=3):
                try:
                    txt = load_text(u)
                    if txt:
                        docs.append(txt[:2000])  # small caps to keep prompt size reasonable
                except Exception:
                    continue

        # If we couldn't infer company, skip RAG & scoring for this lead
        if not docs:
            results.append({**lead, "note": "No company context found, skipping RAG/score."})
            if len(results) >= max_leads:
                break
            continue

        # 3) RAG: Summarize “What does {company} do and who is the ICP?”
        question = f"What does {lead['company']} do? Who is the ideal customer profile?"
